...
func outputToKafka(
	datesInMap []string,
	dateUnixtime map[string]int64,
	container map[string]*hyperloglog.HyperLogLogPlus,
	p kafka.Producer,
	...
	if len(datesInMap) > 2 {
		...
		if dateUnixtime[datesInMap[0]]-dateUnixtime[datesInMap[1]] < 5 {
			key := datesInMap[0]
			encodedHLL, err := container[key].GobEncode()
			logErr(err)
			outputJSON, _ := json.Marshal(OutputMessage{key, encodedHLL})
